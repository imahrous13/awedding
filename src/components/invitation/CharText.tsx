...
export function CharText({
  ...
  // Arabic shaping depends on adjacent characters. Wrapping each Arabic
  // character in its own element breaks joining, so animate Arabic as a unit.
  const containsArabic = /[\u0600-\u06FF]/u.test(text);
  if (connected || containsArabic) {
    return (
